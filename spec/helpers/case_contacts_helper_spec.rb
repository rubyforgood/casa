...
describe CaseContactsHelper do
  ...
  describe "#set_contact_made_true" do
    it "returns true if contact_made is true" do
      case_contact = create(:case_contact, contact_made: true)
      expect(helper.set_contact_made_true(case_contact)).to eq(true)
    end

    it "returns false if contact_made is false" do
      case_contact = create(:case_contact, contact_made: false)
      expect(helper.set_contact_made_true(case_contact)).to eq(false)
    end
  ...
  describe "#set_hours_form" do
    it "returns minutes if duration_minutes is set" do
      case_contact = build(:case_contact, duration_minutes: 80)
      expect(helper.set_hours_form(case_contact)).to eq(1)
    end

    it "returns zero if duration_minutes is zero" do
      case_contact = build(:case_contact, duration_minutes: 0)
      expect(helper.set_hours_form(case_contact)).to eq(0)
    end
