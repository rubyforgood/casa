  ...
  before :all do
    ...
    supervisors.each do |supervisor|
      ...
      volunteers.each_with_index do |volunteer, idx|
        ...
        volunteer.casa_cases << build(:casa_case, :not_transition_aged, casa_org: org)
        volunteer.casa_cases << build(:casa_case, casa_org: org, transition_aged_youth: idx == 1) # TODO remove TAY usage
      end
    end
  ...
  describe "order by" do
    ...
    describe "has_transition_aged_youth_cases" do
      ...
      let(:transition_aged_youth_bool_to_int) do
        lambda { |volunteer|
          volunteer.casa_cases.any? { |x| x.has_transitioned? } ? 1 : 0
        }
      end
