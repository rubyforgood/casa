module TableHelper
  ...
  def sortable_header(label, column, sort:, direction:)
    ...
    # items-start (not items-center) + a one-line-tall caret box keeps the caret pinned to the
    # first line of the label, so every column's caret shares a baseline even when a long label
    # wraps to two lines (items-center would re-center the caret on the taller wrapped label,
    # leaving the row of carets jagged).
    link_class = "inline-flex items-start rounded text-xs font-semibold #{active ? "text-slate-900" : "text-slate-600"} hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
  ...
  def sort_caret(state)
    ...
    raw(%(<span class="inline-flex h-4 items-center" style="margin-left:4px" aria-hidden="true"><svg width="8" height="12" viewBox="0 0 8 12" fill="none"><path d="M0.75 5 L4 2 L7.25 5" stroke="#{up}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M0.75 7 L4 10 L7.25 7" stroke="#{down}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></span>))
  end
