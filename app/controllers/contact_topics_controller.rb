class ContactTopicsController < ApplicationController
  before_action :set_contact_topic, only: %i[edit update destroy]
  ...
  # DELETE /contact_topics/1 or /contact_topics/1.json
  def destroy
    authorize @contact_topic

    if @contact_topic.update(active: false)
  ...
  def contact_topic_params
    params.require(:contact_topic).permit(:casa_org_id, :question, :details)
  end
end
