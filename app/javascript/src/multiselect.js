...
function MultiselectDropdown (options) {
  ...
  document.querySelectorAll('select[multiple].group-select').forEach((el, k) => {
      ...
      div.refresh = () => {
        ...
        if (sels.length > (el.attributes['multiselect-max-items']?.value ?? 5)) {
          ...
          sels.map(x => {
            ...
            if ((el.attributes['multiselect-hide-x']?.value !== 'true')) { c.appendChild(newEl('span', { class: 'optdel', text: 'X', title: config.txtRemove, onclick: (ev) => { c.srcOption.listitemEl.dispatchEvent(new Event('click')); div.refresh(); ev.stopPropagation() } })) }
