...
function checkEmailsAndDisplayPopup(button, email, email_confirmation, enablePopup = false) {
    const emailConfirmation = email.value
    const emailConfirmationText = email_confirmation.value

    if (emailConfirmation === emailConfirmationText) {
...
$('document').ready(() => {
    if ($(`.${SUBMIT_EMAIL_BUTTON_CLASS}`).length > 0) {
        ...
        const email_confirmation = $(`.${EMAIL_CONFIRMATION_FIELD_CLASS}`)[0]

        disableBtn(button)

        $(`.${EMAIL_FIELD_CLASS}`).on('blur', () => {
            checkEmailsAndDisplayPopup(button, email, email_confirmation)
        })

        $(`.${EMAIL_CONFIRMATION_FIELD_CLASS}`).on('blur', () => {
            checkEmailsAndDisplayPopup(button, email, email_confirmation, true)
        })
    }
})
